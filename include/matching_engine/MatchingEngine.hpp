...
public:
    ...
    /// Process a command, appending engine events into @p events (cleared first).
    /// Reuse @p events across calls to avoid per-command vector heap churn.
    void process_into(const OrderCommand& command, std::vector<EngineEvent>& events);

    /// Compatibility wrapper; allocates a new vector each call.
...
private:
    ...
    void process_modify(const OrderCommand& command, std::vector<EngineEvent>& events);

    void execute_new_order(uint64_t order_id, market_data::Side side, OrderType order_type,
                           int64_t price, uint64_t quantity, std::vector<EngineEvent>& events);
    ...
    void match_sell_limit(uint64_t order_id, int64_t price, uint64_t quantity,
                          std::vector<EngineEvent>& events);
    void match_buy_market(uint64_t order_id, uint64_t quantity, std::vector<EngineEvent>& events);
    void match_sell_market(uint64_t order_id, uint64_t quantity, std::vector<EngineEvent>& events);
    ...
    static void reject_into(std::vector<EngineEvent>& events, uint64_t order_id,
                            std::string reason);
