...
step "1/6 Validate the Beat the Market Order challenge"
python3 -m json.tool \
  arena/challenges/beat_market_order.v1.json >/dev/null

step "2/6 Run Arena evaluator and artifact tests"
python3 -m unittest discover -s arena/tests -p 'test_*.py' -v

step "3/6 Build the trusted local C++ example strategy"
cmake -S arena/cpp -B build/arena-cpp
cmake --build build/arena-cpp

step "4/6 Generate built-in strategy result and replay artifacts"
...
step "5/6 Generate C++ strategy result and replay artifacts"
python3 arena/tools/evaluate_execution_v1.py \
  --challenge arena/challenges/beat_market_order.v1.json \
...
step "6/6 Install locked frontend dependencies and build/test the website"
(
