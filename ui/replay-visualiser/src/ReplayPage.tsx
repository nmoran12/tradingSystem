...
type ReplayPageProps = {
  initialReplay?: ArenaReplay | null;
...
export const ReplayPage: React.FC<ReplayPageProps> = ({
  ...
  const initialEpisodeIndex = initialReplay && initialSeed !== undefined
    ? Math.max(
        0,
        initialReplay.episodes.findIndex(
          (episode) => episode.seed === initialSeed,
        ),
      )
    : 0;
  const [replay, setReplay] = useState<ArenaReplay | null>(initialReplay);
  const [source, setSource] = useState<Source>(
    initialSource ? { label: initialSource } : null,
  );
  ...
  useEffect(() => {
    ...
    setSource(initialSource ? { label: initialSource } : null);
    const selectedIndex = initialSeed === undefined
      ? 0
      : initialReplay.episodes.findIndex(
          (item) => item.seed === initialSeed,
        );
    setEpisodeIndex(Math.max(0, selectedIndex));
    setFrameIndex(0);
    setPlaying(false);
    setError(null);
  }, [initialReplay, initialSeed, initialSource]);
  ...
  return (
    <section className={`app-shell ${embedded ? 'embedded-replay' : ''}`}>
      {!embedded && (
        <header className="hero">
          <div>
            ...
            <p className="hero-copy">
              Inspect deterministic replay files generated by the local Arena
              CLI. This page does not run or submit strategy code.
            </p>
          </div>
